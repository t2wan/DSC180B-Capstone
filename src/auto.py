...
def autophrase(filename1,filename2,save_path,direct_path,raw_path):
    mycwd = os.getcwd()
    os.rename(raw_path+filename1+'.txt', direct_path+filename1+'.txt')
    os.chdir('AutoPhrase')
...
DEFAULT_TRAIN=${DATA_DIR}/EN/'''+filename1+'''.txt
...
### END Generating Output for Checking Quality ###
    ...
    subprocess.run(['./auto_phrase.sh',filename1+'.txt'])

    os.chdir(mycwd)
    os.rename('./AutoPhrase/models/DBLP/AutoPhrase.txt', save_path+filename1+' AutoPhrase.txt')
    os.rename('./AutoPhrase/models/DBLP/AutoPhrase_multi-words.txt', save_path+filename1+' AutoPhrase_multi-words.txt')
    os.rename('./AutoPhrase/models/DBLP/AutoPhrase_single-word.txt', save_path+filename1+' AutoPhrase_single-word.txt')
    os.rename('./AutoPhrase/models/DBLP/token_mapping.txt', save_path+filename1+' token_mapping.txt')
    os.rename('./AutoPhrase/models/DBLP/segmentation.model', save_path+filename1+' segmentation.model')
    os.rename(direct_path+filename1+'.txt',raw_path+filename1+'.txt')

    mycwd = os.getcwd()
    os.rename(raw_path+filename2+'.txt', direct_path+filename2+'.txt')
    os.chdir('AutoPhrase')
    os.remove('auto_phrase.sh')
    with open ('auto_phrase.sh', 'w') as rsh:
        rsh.write('''\
...
MODEL=${MODEL:- ${MODELS_DIR}/DBLP}
# RAW_TRAIN is the input of AutoPhrase, where each line is a single document.
DEFAULT_TRAIN=${DATA_DIR}/EN/'''+filename2+'''.txt
RAW_TRAIN=${RAW_TRAIN:- $DEFAULT_TRAIN}
# When FIRST_RUN is set to 1, AutoPhrase will run all preprocessing. 
# Otherwise, AutoPhrase directly starts from the current preprocessed data in the tmp/ folder.
FIRST_RUN=${FIRST_RUN:- 1}
...
### END Generating Output for Checking Quality ###
    ...
    subprocess.run(['chmod','+x','auto_phrase.sh'])
    subprocess.run(['./auto_phrase.sh',filename2+'.txt'])

    os.chdir(mycwd)
    os.rename('./AutoPhrase/models/DBLP/AutoPhrase.txt', save_path+filename2+' AutoPhrase.txt')
    os.rename('./AutoPhrase/models/DBLP/AutoPhrase_multi-words.txt', save_path+filename2+' AutoPhrase_multi-words.txt')
    os.rename('./AutoPhrase/models/DBLP/AutoPhrase_single-word.txt', save_path+filename2+' AutoPhrase_single-word.txt')
    os.rename('./AutoPhrase/models/DBLP/token_mapping.txt', save_path+filename2+' token_mapping.txt')
    os.rename('./AutoPhrase/models/DBLP/segmentation.model', save_path+filename2+' segmentation.model')
    os.rename(direct_path+filename2+'.txt',raw_path+filename2+'.txt')
